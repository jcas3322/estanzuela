import React, { useRef, useState } from 'react'
import {
  SafeAreaView,
  View,
  StyleSheet,
  Text,
  TextInput,
  ScrollView,
  Button,
  Alert
} from 'react-native'
import TakenImages from '../TakenImages'
import axios from 'axios'
import { API_BASE_URL } from '../../constants/constantes'

const RegisterCiudadado = ({setModalVisible}) => {

    const [nombres, setNombres] = useState('')
    const [dpi, setDpi] = useState('')
    const [telefono, setTelefono] = useState('')
    const [direccion, setDireccion] = useState('')
    const [email, setEmail] = useState('')
    const [foto, setFoto] = useState('data:image/png;base64,')
    const [password, setPassword] = useState('')
    const [retypePassword, setTRetypePassword] = useState('')
    const [subirFoto, setSubirFoto] = useState(false)
    const inputRef = useRef(null)

    const base64Foto = 'data:image/png;base64,'

    const devolverFoco = () => {
        inputRef.current.focus()
      }

      const subirCiudadano = async () => {
        if ([nombres, dpi, direccion, telefono, email, password, retypePassword].includes('')) {
          Alert.alert('Error', 'Todos los campos son obligatorios')
          return false
        }
        if(password !== retypePassword){
            Alert.alert('Error', 'La contraseña no coincide')
            return false  
        }

        if(subirFoto){
            try{
                const formData = new FormData()
                formData.append('foto',{
                    uri: foto,
                    name:'foto.jpg',
                    type: 'image/jpg'
                })
                const response = await axios.post(`${API_BASE_URL}/user/saveFoto`, formData, {
                    headers: {
                        'Content-Type': 'multipart/form-data'
                    }
                })
                if(response.status === 201){
                    objetoCliente.foto = response.data.url
                    const response2 = await axios.post(`${API_BASE_URL}/user/signup`, objetoCliente)
                    Alert.alert('Guardado', 'Ciudadano registrado')
                    limpiarFormulario()
                    setModalVisible(false)
                }
            }catch(e){
                Alert.alert('Error', e)
            }
        }else{
            const response2 = await axios.post(`${API_BASE_URL}/user/signup`, objetoCliente)
            Alert.alert('Guardado', 'Ciudadano registrado')
            limpiarFormulario()
            setModalVisible(false)
        }
      }
    
      const objetoCliente = {
        nombres: nombres,
        dpi: dpi,
        direccion: direccion,
        telefono: telefono,
        email: email,
        foto: '',
        password: password,
      }
    
      const limpiarFormulario = () => {
        setNombres('')
        setDireccion('')
        setDpi('')
        setTelefono('')
        setEmail('')
        setFoto(base64Foto)
        setPassword('')
        setTRetypePassword('')
        devolverFoco()
      }
  
    return (
        <SafeAreaView style={styles.contenedor}>
        <ScrollView>
          <View style={styles.titulo}>
            <Text style={styles.tituloText1}>Registro de {''}<Text style={[styles.tituloText1, styles.tituloText2]}>Ciudadano</Text></Text>
          </View>
          <View style={styles.campos}>
            <Text style={styles.campoLabels}>Nombres</Text>
            <TextInput
              ref={inputRef}
              style={styles.campoInputs}
              placeholder='Ingrese sus nombres'
              placeholderTextColor={'#666'}
              value={nombres}
              onChangeText={setNombres}
            />
          </View>
          <View style={styles.campos}>
            <Text style={styles.campoLabels}>DPI</Text>
            <TextInput
              style={styles.campoInputs}
              placeholder='Ingrese su DPI'
              placeholderTextColor={'#666'}
              value={dpi}
              onChangeText={setDpi}
            />
          </View>
          <View style={styles.campos}>
            <Text style={styles.campoLabels}>Direccion</Text>
            <TextInput
              style={styles.campoInputs}
              placeholder='Ingrese su direccion'
              placeholderTextColor={'#666'}
              value={direccion}
              onChangeText={setDireccion}
              maxLength={10}
            />
          </View>
          <View style={styles.campos}>
            <Text style={styles.campoLabels}>Telefono</Text>
            <TextInput
              style={styles.campoInputs}
              placeholder='Ingrese telefono'
              placeholderTextColor={'#666'}
              value={telefono}
              onChangeText={setTelefono}
              maxLength={10}
            />
          </View>
          <View style={styles.campos}>
            <Text style={styles.campoLabels}>Email</Text>
            <TextInput
              style={styles.campoInputs}
              placeholder='Ingrese su email'
              placeholderTextColor={'#666'}
              value={email}
              onChangeText={setEmail}
            />
          </View>
          <View style={styles.campos}>
            <Text style={styles.campoLabels}>Contraseña</Text>
            <TextInput
              style={styles.campoInputs}
              placeholder='Ingrese su contraseña'
              placeholderTextColor={'#666'}
              value={password}
              onChangeText={setPassword}
            />
          </View>
          <View style={styles.campos}>
            <Text style={styles.campoLabels}>Repita contraseña</Text>
            <TextInput
              style={styles.campoInputs}
              placeholder='Repita su contraseña'
              placeholderTextColor={'#666'}
              value={retypePassword}
              onChangeText={setTRetypePassword}
              maxLength={10}
            />
          </View>
          <TakenImages
            foto={foto}
            setFoto={setFoto}
            subirFoto={setSubirFoto}
          />
          <View style={styles.btnGuardar}>
            <Button
              title='Registrarse'
              onPress={subirCiudadano}
            />
          </View>
          <View style={styles.btnGuardar}>
            <Button
                title='Regresar'
                onPress={()=>setModalVisible(false)}
            />
          </View>
        </ScrollView>
      </SafeAreaView>
  )
}

const styles = StyleSheet.create({
  contenedor: {
    flex: 1,
    backgroundColor: '#D9F0FF',
    paddingHorizontal: 24,
  },
  titulo: {
    marginTop: 50
  },
  tituloText1: {
    textAlign: 'center',
    fontSize: 25
  },
  tituloText2: {
    color: '#751693',
    fontWeight: '700'
  },
  campos: {
    marginTop: 15
  },
  campoLabels: {
    textTransform: 'uppercase',
    fontSize: 18
  },
  campoInputs: {
    backgroundColor: '#fff',
    marginTop: 10,
    borderRadius: 5,
    fontSize: 16
  },
  btnGuardar: {
    marginVertical:15
}
});

export default RegisterCiudadado